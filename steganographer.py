...
class Steganographer:
    def __init__(self, image_path):
        ...
        # Гарантируем правильный тип данных
    ...
    def embed_basic(self, text, seed):
        ...
        # Конвертируем в int32 для безопасных вычислений
        ...
        # Возвращаем к uint8 перед созданием изображения
    ...
    def embed_enhanced(self, text, seed):
        # 1. Кодируем длину сообщения (32 бита)
        ...
        # 2. Объединяем длину + данные
        all_bits = np.concatenate([length_bits, text_bits])
        # 3. Генерация ключа для ТОЧНОЙ длины
        key_text = self.generate_key(seed, 112)#len(all_bits))
        print("Биты", all_bits)
        print("Встроенная длинна", len(text_bits))
        # 3. Шифрование текста
        encoded_text = np.bitwise_xor(all_bits, key_text)
        
        # 4. Разбиение на блоки и добавление хэшей
        block_size = 64
        ...
        # 5. Генерация ОТДЕЛЬНОГО ключа для полной длины (текст + хэши)
        key_full = self.generate_key(seed, len(enhanced_data))
        
        # 6. Дополнительное шифрование всего потока
        final_bits = np.bitwise_xor(enhanced_data, key_full)
        
        # 7. Встраивание в изображение
    ...
    def calculate_capacity(self, text, method="enhanced"):
        ...
        if method == "basic":
            ...
            # Для метода с хэшем: добавляем 16 бит на каждый 64-битный блок
            blocks = (text_bits + 63) // 64  # Округление вверх
            return text_bits + blocks * 16
    
    def extract_basic(self, seed, length_bits):
        # if length_bits is None:
        # length_bits = self.pixels.size  # Прочитает всё, но может быть мусор в конце
        flat_pixels = self.pixels.flatten()
    ...
    def compare_containers(self, original_image_path, stego_image_path):
        ...
        if len(original.shape) != len(stego.shape):
            if len(original.shape) == 3:  # Если оригинал RGB, а стето - Grayscale
                stego = np.stack((stego,)*3, axis=-1)  # Конвертируем в RGB
            else:
        ...
        # Преобразуем в int32 для безопасного вычитания
    ...
    def analyze_lsb_distribution(self, block_size=8):
        ...
        if len(self.pixels.shape) == 3:  # RGB изображение
            lsb = self.pixels[:, :, 0] & 1  # Анализируем только красный канал
        else:  # Grayscale
            lsb = self.pixels & 1
    ...
    def chi_square_test(self, block_size=64):
        ...
        """
        # Получаем LSB всех пикселей
        if len(self.pixels.shape) == 3:  # RGB изображение
            lsb = self.pixels[:, :, 0].flatten() & 1  # Анализируем только красный канал
        else:  # Grayscale
            lsb = self.pixels.flatten() & 1
        
        # Ограничиваем анализ первыми block_size битами
        lsb = lsb[:block_size]
        
        # Группируем биты в пары (2n, 2n+1)
        pairs = [(lsb[i], lsb[i+1]) for i in range(0, len(lsb)-1, 2)]
        
        # Считаем наблюдаемые частоты комбинаций
        freq = {
            (0, 0): 0,
            (0, 1): 0,
            (1, 0): 0,
        ...
        # Вычисляем ожидаемые частоты (для естественного изображения)
        total_pairs = len(pairs)
        if total_pairs == 0:
            return 1.0  # Если нет данных для анализа
        ...
        # Вычисляем p-value (используем scipy.stats.chi2)
    ...
    def advanced_analysis(self):
        ...
        if len(self.pixels.shape) == 3:  # RGB
